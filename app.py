...
app.config['SQLALCHEMY_DATABASE_URI'] = (
    os.environ.get('DATABASE_URL', 'postgresql:///foodie'))
...
def signup():
    """Handle user signup"""
    if g.user:
        flash("You are already logged in. Want to create another account? Logout first.", "warning")
...
def home_page():
    ...
    response = requests.request(
        "GET", url + randomFind, params=querystring).json()
...
def get_recipe():
    if (str(request.args['search']).strip() != ""):
        queryString = {
            "query": request.args['search'], "number": 20, "apiKey": apiKey}
        response = requests.request(
            "GET", url + search, params=queryString).json()
...
def get_recipes_by_ingredient():
    if (str(request.args['ingrdt']).strip() != ""):
        querystring = {"number": "20", "ranking": "1",
                       "ingredients": request.args['ingrdt'], "apiKey": apiKey}
        response = requests.request(
            "GET", url + find, params=querystring).json()
...
def get_recipe_detail():
    ...
    recipe_info = requests.request(
        "GET", url + recipe_info_endpoint, params=queryKey).json()
    querystring = {"defaultCss": "true",
                   "showBacklink": "false", "apiKey": apiKey}
    food = {'food': recipe_info['title'], 'apiKey': apiKey}
    recipe_info['inregdientsWidget'] = requests.request(
        "GET", url + ingedientsWidget, params=querystring).text
    recipe_info['equipmentWidget'] = requests.request(
        "GET", url + equipmentWidget, params=querystring).text
    recipe_info['winePairing'] = requests.request(
        "GET", url + winePairing, params=food).json()
...
def get_recipes_by_nutrient():
    ...
    recipe_info = requests.request(
        "GET", url + findByNutrient, params=queryString).json()
...
def add_favorite():
    ...
    new_fav = Favorite(recipe_id=recipe_id, title=recipe_title,
                       image=recipe_image, user_id=user_id)
...
def add_menu():
    ...
    if form.validate_on_submit():
        ...
        new_menu = Menu(day=day, time=time, title=title,
